...
from docx import Document # Word desteği için eklendi
    ...
    if not st.session_state.kategoriler:
        ...
        if format_secimi == "PDF":
            ...
            if kdv_ekle:
                ...
                pdf.cell(45, 5, p(f"{ara_toplam:,.0f} {para_birimi}"), ln=1, align='C')
                pdf.cell(110, 5, "", ln=0)
                pdf.cell(35, 5, p(f"KDV(%{kdv_oran})"), ln=0, align='R')
                pdf.cell(45, 5, p(f"{kdv_tutari:,.0f} {para_birimi}"), ln=1, align='C')
            
            pdf.cell(110, 5, "", ln=0)
            pdf.cell(35, 5, "Genel Toplam", ln=0, align='R')
            pdf.cell(45, 5, p(f"{genel_toplam:,.0f} {para_birimi}"), ln=1, align='C')

            if st.session_state.sartlar:
                pdf.ln(10); pdf.set_text_color(*BLUE); pdf.set_font(font_adi, '', 14)
                pdf.cell(0, 8, p("Genel Şartlar"), ln=1)
                pdf.set_draw_color(*BLUE); pdf.line(10, pdf.get_y(), 200, pdf.get_y())
                pdf.set_draw_color(*BLACK); pdf.set_text_color(*BLACK); pdf.set_font(font_adi, '', 9); pdf.ln(2)
        ...
        # --- WORD OLUŞTURMA (.docx) ---
            ...
            doc.add_heading(t_baslik2, level=1)
            doc.add_paragraph(f"Tarih: {t_tarih}")
            
            # Müşteri Bilgileri
            doc.add_heading('Müşteri Bilgileri', level=2)
            doc.add_paragraph(f"Firma/Kişi: {m_ad}\nTel: {m_tel}\nE-Posta: {m_mail}")
            ...
            for k in st.session_state.kategoriler:
                p = doc.add_paragraph()
                p.add_run(k['baslik']).bold = True
                doc.add_paragraph(k['aciklama'])
                doc.add_paragraph(f"Adet: {k['adet']} | Birim Fiyat: {k['fiyat']:,.0f} {para_birimi} | Toplam: {k['toplam']:,.0f} {para_birimi}")
                ara_toplam += k['toplam']
            ...
            doc.add_paragraph("\n")
            doc.add_paragraph(f"Ara Toplam: {ara_toplam:,.0f} {para_birimi}")
            if kdv_ekle:
                doc.add_paragraph(f"KDV (%{kdv_oran}): {kdv_tutari:,.0f} {para_birimi}")
            doc.add_paragraph(f"GENEL TOPLAM: {genel_toplam:,.0f} {para_birimi}").bold = True
            
            # Şartlar
            if st.session_state.sartlar:
                doc.add_heading('Genel Şartlar', level=2)
                for sart in st.session_state.sartlar:
                    doc.add_paragraph(sart, style='List Bullet')
            
            # Word dosyasını indirilebilir hale getir
            buffer = io.BytesIO()
            doc.save(buffer)
            buffer.seek(0)

...
st.title("Teklif Hazırlama Sistemi")

if 'kategoriler' not in st.session_state:
    st.session_state.kategoriler = []
    
# VARSAYILAN GENEL ŞARTLAR (TÜRKÇE KARAKTERLİ)
...
# --- 3. GENEL ŞARTLAR (DİNAMİK) ---
st.subheader(" Genel Şartlar")
...
if st.button(" TEKLİFİ OLUŞTUR VE İNDİR", type="primary"):
    if not st.session_state.kategoriler:
        ...
        # --- TÜRKÇE FONT TANITIMI (Windows Orijinal Fontları) ---
        ...
        pdf.set_xy(10, 15)
        pdf.cell(0, 4.5, "Alpinetech Mühendislik Makine", ln=1)
        pdf.cell(0, 4.5, "Sanayi ve Ticaret Limited Şirketi", ln=1)
        pdf.cell(0, 4.5, "30 Ağustos Zafer Mahallesi 520.", ln=1)
        pdf.cell(0, 4.5, "Cadde 4C/A Nilüfer Bursa", ln=1)
        ...
        pdf.cell(0, 8, f"TEKLİF: {t_baslik1}", ln=1)
        
        pdf.set_font("ArialTR", 'B', 14)
        pdf.cell(100, 8, t_baslik2, ln=0)
        pdf.set_font("ArialTR", '', 11)
        pdf.cell(90, 8, t_tarih, ln=1, align='R')
        
        pdf.set_line_width(0.3)
        pdf.line(10, pdf.get_y()+2, 200, pdf.get_y()+2)
        pdf.ln(5)

        # MÜŞTERİ VE TEKLİF VEREN KISMI
        pdf.set_font("ArialTR", 'B', 9)
        y_info = pdf.get_y()
        
        # Sol Taraf
        pdf.set_xy(10, y_info)
        pdf.cell(15, 5, "Müşteri", ln=0); pdf.set_font("ArialTR", ''); pdf.cell(70, 5, m_ad, ln=1)
        pdf.set_font("ArialTR", 'B'); pdf.cell(15, 5, "Telefon", ln=0); pdf.set_font("ArialTR", ''); pdf.cell(70, 5, m_tel, ln=1)
        pdf.set_font("ArialTR", 'B'); pdf.cell(15, 5, "E-Posta", ln=0); pdf.set_font("ArialTR", ''); pdf.cell(70, 5, m_mail, ln=1)
        
        # Sağ Taraf
        pdf.set_font("ArialTR", 'B', 9)
        pdf.set_xy(115, y_info)
        pdf.cell(25, 5, "Teklifi veren", ln=0); pdf.set_font("ArialTR", ''); pdf.cell(50, 5, v_ad, ln=1)
        pdf.set_xy(115, y_info+5)
        pdf.set_font("ArialTR", 'B', 9); pdf.cell(25, 5, "Telefon", ln=0); pdf.set_font("ArialTR", ''); pdf.cell(50, 5, v_tel, ln=1)
        pdf.set_xy(115, y_info+10)
        pdf.set_font("ArialTR", 'B', 9); pdf.cell(25, 5, "E-posta", ln=0); pdf.set_text_color(*GREEN); pdf.cell(50, 5, v_mail, ln=1)
        pdf.set_text_color(*BLACK)
        ...
        for k in st.session_state.kategoriler:
            ...
            pdf.cell(0, 6, f"Fiyat : {kutu_fiyat} {para_birimi}", border=1, align='R', ln=1)
            pdf.ln(5)
            ara_toplam += k['toplam']
        ...
        for k in st.session_state.kategoriler:
            ...
            pdf.cell(40, 5, "", ln=0) 
            pdf.cell(50, 5, k['baslik'], ln=0, align='R') 
        ...
        if kdv_ekle:
            ...
            pdf.cell(110, 5, "", ln=0)
            pdf.cell(35, 5, f"KDV(%{kdv_oran})", ln=0, align='R')
        ...
        # --- DİNAMİK GENEL ŞARTLAR ---
        if st.session_state.sartlar:
            pdf.ln(10)
            pdf.set_text_color(*BLUE)
            pdf.set_font("ArialTR", '', 14)
            pdf.cell(0, 8, "Genel Şartlar", ln=1)
            pdf.set_draw_color(*BLUE)
            pdf.line(10, pdf.get_y(), 200, pdf.get_y())
            pdf.set_draw_color(*BLACK)
            ...
            for sart in st.session_state.sartlar:
                pdf.cell(5, 5, "-", ln=0)
                pdf.multi_cell(0, 5, sart)

        # BOŞ SAYFA HATASI İÇİN ÖZEL ÇÖZÜM
        pdf.set_auto_page_break(False) # Sayfa numarasını basmadan önce otomatik yeni sayfa açmayı kapat
        pdf.set_y(-15)

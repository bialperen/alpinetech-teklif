    ...
    if not st.session_state.kategoriler:
        ...
        if format_secimi == "PDF":
            ...
            for k in st.session_state.kategoriler:
                ...
                # Resimleri PDF'e ekleme (Örnek resimdeki gibi yan yana)
                if k['resimler']:
                    pdf.ln(2)
                    y_start = pdf.get_y()
                    ...
                    pdf.set_y(y_start + 65) # Resim boyutu kadar aşağı in
            ...
            # --- GENEL TOPLAM TABLOSU VE ŞARTLAR (PDF) ---
            pdf.add_page() # Genel toplam ve şartları her zaman yeni sayfaya al
            pdf.set_text_color(*BLUE)
        ...
        # --- WORD OLUŞTURMA (.docx) ---
            ...
            t1 = title_p.add_run(f"TEKLİF: {t_baslik1}")
            t1.bold = True
            t1.font.size = Pt(16)
            ...
            for k in st.session_state.kategoriler:
                ...
                if k['resimler']:
                    # Resimleri Word'e yan yana eklemek için tablo kullanımı
                ...
                p_fiyat = doc.add_paragraph(f"\nBirim Fiyat: {k['fiyat']:,.0f} {para_birimi} | Toplam: {k['toplam']:,.0f} {para_birimi}")
                p_fiyat.alignment = WD_ALIGN_PARAGRAPH.RIGHT

            # Genel Toplam ve KDV Tablosu
            doc.add_page_break()
            ...
            hdr_cells[0].text = 'İş Kalemi'
            hdr_cells[1].text = 'Adet'
            hdr_cells[2].text = 'Birim Fiyat'
            hdr_cells[3].text = 'Toplam'
            
            for k in st.session_state.kategoriler:
                row_cells = table.add_row().cells
                row_cells[0].text = k['baslik']
                row_cells[1].text = str(k['adet'])
                row_cells[2].text = f"{k['fiyat']:,.0f} {para_birimi}"
                row_cells[3].text = f"{k['toplam']:,.0f} {para_birimi}"
            ...
            # ŞARTLARIN WORD'E EKLENDİĞİ YER
            if st.session_state.sartlar:
